...
def temperature_scale(logits, model, new_past, config, temperature, temperature_horizon):
    ...
    # EXTRA CREDIT ONLY   
        ...
        first_probs = None
        ### START CODE HERE ###
        first_probs = F.softmax(logits, dim=-1)    # shape (1, vocab_size)
        ### END CODE HERE ###
        ...
        for t in range(config.vocab_size):
            ...
            filtered2     = top_k_logits(second_logits, k=config.top_k)
            second_probs  = F.softmax(filtered2, dim=-1)
            joint_prob_t  = first_prob * second_probs   # shape (1, vocab_size)
            ### END CODE HERE ###
        ...
        first_logits = None
        ### START CODE HERE ###
        joint_logits_scaled = joint_logits / temperature
        first_logits        = torch.logsumexp(joint_logits_scaled, dim=1)
        ### END CODE HERE ###
